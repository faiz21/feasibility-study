# Step 10 - Enriched Instruction Addendum

Global reference: `00-global-enriched-instruction.md`
Base schema: `plant-report-generator-enriched-base.schema.json`
Target schema: `plant-report-generator-10-enriched.schema.json`

## Intent
Produce report-ready transformed data for **Appendix I — Procedural Scope: Procedures Covered** with consultant-grade 360 analysis, maturity scoring (0-100 + 5-level label), executive narrative packaging, and visualization-ready chart definitions.

## Required Inputs
- Source instruction: `plant-report-generator-10.md`
- Source schema: `plant-report-generator-10.json`
- Prerequisite steps: 1, 2, 3, 4, 5, 6, 7, 8, 9
- Evidence sources:
...
- Preserve deterministic logic and stop conditions from Step 10 baseline artifacts.
- Map baseline step structure directly into `step_payload` with required keys: `document_id_pattern`, `prerequisite_steps`, `evidence_sources`, `category_classification_precedence`, `sections`, `outputs`, `quality_gate`.
...
- Procedure table missing or empty
- Procedure introduced not in authoritative table
- Classification lacks deterministic keyword trigger
- Gap statement lacks evidence anchor
- Maturity contradicts deterministic triggers

## Output Contract
- Output object must validate against `plant-report-generator-10-enriched.schema.json`.
- `source_step` must be `10` and `report_profile.title` must be exactly `Appendix I — Procedural Scope: Procedures Covered`.
