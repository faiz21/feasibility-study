# Step 03 - Enriched Instruction Addendum

Global reference: `00-global-enriched-instruction.md`
Base schema: `plant-report-generator-enriched-base.schema.json`
Target schema: `plant-report-generator-03-enriched.schema.json`

## Intent
Produce report-ready transformed data for **Appendix B — Process Architecture: Process Groups Inventory** with consultant-grade 360 analysis, maturity scoring (0-100 + 5-level label), executive narrative packaging, and visualization-ready chart definitions.

## Required Inputs
- Source instruction: `plant-report-generator-03.md`
- Source schema: `plant-report-generator-03.json`
...
- Preserve deterministic logic and stop conditions from Step 3 baseline artifacts.
...
- Process Groups table missing
- Introduced group not present in dataset
- PLC linkage contradicts Process Group mapping
- Lifecycle interpretation uses non-numeric year

## Output Contract
- Output object must validate against `plant-report-generator-03-enriched.schema.json`.
- `source_step` must be `3` and `report_profile.title` must be exactly `Appendix B — Process Architecture: Process Groups Inventory`.
