# Step 06 - Enriched Instruction Addendum

Global reference: `00-global-enriched-instruction.md`
Base schema: `plant-report-generator-enriched-base.schema.json`
Target schema: `plant-report-generator-06-enriched.schema.json`

## Intent
Produce report-ready transformed data for **Appendix E — Supervisory Control Scope: SCADA/DCS Inventory** with consultant-grade 360 analysis, maturity scoring (0-100 + 5-level label), executive narrative packaging, and visualization-ready chart definitions.

## Required Inputs
- Source instruction: `plant-report-generator-06.md`
- Source schema: `plant-report-generator-06.json`
- Prerequisite steps: 1, 2, 3, 4, 5
- Evidence sources:
- `List Of SCADA`
- `List Of PLC`
...
- Preserve deterministic logic and stop conditions from Step 6 baseline artifacts.
...
- SCADA code referenced by PLC not found
- Year Built non-numeric
- Coverage contradicts PLC linkage
- Historian presence asserted without keyword evidence
- Narrative introduces systems not in dataset

## Output Contract
- Output object must validate against `plant-report-generator-06-enriched.schema.json`.
- `source_step` must be `6` and `report_profile.title` must be exactly `Appendix E — Supervisory Control Scope: SCADA/DCS Inventory`.
