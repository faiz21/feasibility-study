# Step 09 - Enriched Instruction Addendum

Global reference: `00-global-enriched-instruction.md`
Base schema: `plant-report-generator-enriched-base.schema.json`
Target schema: `plant-report-generator-09-enriched.schema.json`

## Intent
Produce report-ready transformed data for **Appendix H — Functional Scope: Function Groups / Value Streams** with consultant-grade 360 analysis, maturity scoring (0-100 + 5-level label), executive narrative packaging, and visualization-ready chart definitions.

## Required Inputs
- Source instruction: `plant-report-generator-09.md`
- Source schema: `plant-report-generator-09.json`
- Prerequisite steps: 1, 2, 3, 4, 5, 6, 7, 8
- Evidence sources:
...
- Preserve deterministic logic and stop conditions from Step 9 baseline artifacts.
...
- Function Group table missing or empty
- Function Group introduced not in authoritative table
- Gap lacks evidence anchor
- Maturity contradicts deterministic triggers

## Output Contract
- Output object must validate against `plant-report-generator-09-enriched.schema.json`.
- `source_step` must be `9` and `report_profile.title` must be exactly `Appendix H — Functional Scope: Function Groups / Value Streams`.
